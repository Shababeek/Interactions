# Constrained Interactables — Lever, Drawer, Slider, Joystick, Wheel, Dial
...
| **Drawer** | Linear slide between points | 1 | Drawers, sliding doors |
| **Slider** | Linear slide with discrete steps | 1 | Volume rails, detented faders |
...
**Create with placeholder model** — Right-click in Hierarchy > **Shababeek > Create > [Lever / Drawer / Joystick / Wheel / …]** (availability depends on installed menu items). This creates a complete interactable with a placeholder model, ready to test. The object is parented under the selected object at local (0,0,0), or placed at the scene root.
...
**Add Component manually** — Select object > **Add Component** → **Shababeek → Interactions → Interactables → [Lever / Drawer / Slider / Joystick / Wheel / Dial]**.
...
> **Menu Path:** Component > Shababeek > Interactions > Interactables > Slider  
> **Use For:** Linear controls that snap to discrete steps (e.g. volume-style sliders, stepped linear selectors)

## What It Does

**Slider Interactable** extends the shared linear constrained base (same lineage as **Drawer**) but is tuned for **multiple discrete steps** along one axis. While grabbed, the knob follows the hand; on release it snaps to the nearest step.

![Slider Interactable Inspector (placeholder)](../Images/Constrained_Interactables/slider-interactable-inspector.png)

## Inspector essentials

| Field | Tip |
|-------|-----|
| **Number Of Steps** | Must be ≥ 2; defines how many detented positions exist |
| **Starting Step** | Initial index when the scene loads |
| **Interactable Object** | The moving mesh / collider that slides |

Use **Uniform scale** on the interactable root (see warning at top of this page).
