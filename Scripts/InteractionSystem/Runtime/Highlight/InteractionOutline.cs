...
//  Based on QuickOutline (MIT) — https://github.com/chrisnolet/QuickOutline
//  Renamed and namespaced for the Shababeek Interaction System to avoid clashing with a customer's own QuickOutline copy.
//
...
namespace Shababeek.Interactions.Highlight
{
  ...
  /// <summary>
  /// Mesh outline / silhouette renderer used by the interaction highlight feedback components.
  /// </summary>
  public class InteractionOutline : MonoBehaviour {
    ...
    void DetachMaterials() {
      ...
      for (int i = 0; i < renderers.Length; i++) {
        var renderer = renderers[i];
        if (renderer == null) continue;
        var original = originalSharedMaterials[i];
    ...
    void EnsureRuntimeResources() {
      ...
      if (outlineFillMaterial == null) {
        var source = Resources.Load<Material>(@"Materials/OutlineFill");
        if (source == null) return;
        outlineFillMaterial = Instantiate(source);
        outlineFillMaterial.name = "OutlineFill (Instance)";
        needsUpdate = true;
      }
    }
